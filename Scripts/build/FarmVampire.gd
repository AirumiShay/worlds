...
#Это хутор с жителями вампирами
...
#	$Owner.text = owner_name + ":" + name_farm
	...
	if data_start !={}:	
		new_position = data_start["new_position"]
func prepare_data_start(object_data):
	data_start={"new_position":0}
	data_start["new_position"] = new_position
	return data_start
func _process(delta):
#	rpc("destroy_self")
#	if spawn == false:
...
func rpc_spawn_mobs(): #(nickname:String, data:Dictionary, die_position:Vector2, url_data:String="") -> void:
	if get_tree().is_network_server():
		var object_path ='res://Scenes/Race/Peasant2.tscn'
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,$Sprite.global_position,"Peasant2",Global_DataParser.join_race,location_number)
...
#		Global_DataParser.build_farm += 1
		var a:Dictionary = {}
		var transfer = prepare_data_start(a)		
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"FarmVampire",Global_DataParser.join_race,location_number,transfer)
...
func rpc_spawn_fortress(posXY): 
	if get_tree().is_network_server():	 
		var object_path = "res://Scenes/Build/VampireFortress.tscn"
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"VampireFortress",Global_DataParser.join_race,location_number)
...
func rpc_spawn_storage(posXY): 
	if get_tree().is_network_server():	 
		var object_path = "res://Scenes/Build/Temple.tscn"
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"Storage",Global_DataParser.join_race,location_number)
...
func rpc_spawn_magtower(posXY): 
	if get_tree().is_network_server():	 
		...
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"MagicTower",Global_DataParser.join_race,location_number)
...
#	if(get_tree().is_network_server()): 
		...
		if new_position < 9:
			if new_position == 1:
				a = x  * 256
				b = (y - 1) * 256
#				spawn_magtower(Vector2(a,b))
			if new_position == 2:
				a = (x - 2)  * 256
				b = y  * 256
#				spawn_storage(Vector2(a,b))
			if new_position == 3:
				a = (x) * 256
				b = (y + 4)  * 256
...
#				spawn_village("Village",Vector2(a,b))
			if new_position == 6:
				a = (x)  * 256
				b = (y + 3)  * 256
#				spawn_fortress("Village",Vector2(a,b))
...
func spawn_village(posXY): #nickname:String, die_position:Vector2): #, url_data:String="") -> void:
	if get_tree().is_network_server():	 
		...
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"FortressEmpire",Global_DataParser.join_race,location_number)
	return
...
func rpc_spawn_temple(posXY): 
	if get_tree().is_network_server():	 
		...
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"TempleEmpire",Global_DataParser.join_race,location_number)
...
func rpc_spawn_castle(posXY): 
	if get_tree().is_network_server():	 
		var object_path = 'res://Scenes/Build/VampireCastle.tscn'
		Global_DataParser.build_castle_vampire += 1			
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"VampireCastle",Global_DataParser.join_race,location_number)
...
func reload_data_by_inventory(inventory:Control) -> Dictionary:
	_data = inventory.load_data(_name, "VampireFarm")
	return _data
...
#			already_build = true	
	...
	#				rpc_spawn_farm(Vector2(a,b))
				...
				if what_do == 1:
					...
					if now_city == 0:
#						$Sprite.texture = load("res://Assets/build//village_green.png")														
#						$Sprite.scale.x = 4
#						$Sprite.scale.y = 4
						now_city = 1
						energy_farm = 100		
...
#						level_farm2 += 1
					...
					rpc_spawn_storage(Vector2(a,b))
				elif what_do == 4:
					rpc_spawn_magtower(Vector2(a,b))															
				SoundPlayer.play(preload("res://audio/sounds/preload_sfx/frost_cast1.wav"))	
...
func get_data():
	...
	_new_data["whoiam"] ="res://Scenes/Build/FarmVampire.tscn"
	_new_data["nickname"] ="FarmVampire"
...
func _on_WhatTimer_timeout():
	...
	#				new_position += 1
		...
		elif what_do == 0 and resource > 550 and (peasant_count > 16) and now_city < 2:
					resource -= 100
	#				peasant_count -= 8
	#				var x = int(position.x/256) - 1
	#				var y = int(position.y/256)	- 1
	...
	#				new_position += 1
					SoundPlayer.play(preload("res://audio/sounds/preload_sfx/fire_cast1.wav"))	
					level_farm2 += 1
					$Sprite.texture = load("res://Assets/build/village_blue.png")														
