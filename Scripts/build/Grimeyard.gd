...
#это постройка - кладбище, на нём рождаются призраки
...
var spawn = false
var spawn_time = 10 #время рождения призраков
var client = 0
var npc_god = false
...
func set_target(target1):
	target_main = target1
	print(target1)
func rpc_spawn_ghost():
	if get_tree().is_network_server():	 
		var object_path = 'res://Scenes/Race/Zombi.tscn'
...
#	var town = load('res://Scenes/AngryAnimal.tscn').instance()
	var	town = load('res://Scenes/Race/Ghost_low.tscn').instance()	
	town.name = nickname + "_" + str(Global_DataParser.rng.randi())
#	print(town.name)
	print("spawning Ghost in Grimeyard")
#	bag.name = nickname
	$'/root/Game/Empire'.add_child(town)
	town.position = target_main + Vector2(-64, -64) #$Sprite.global_position
	town.set_target(target_main)		
...
#	town.reload_data_by_inventory(_inventory)
	...
	var	town1 = load('res://Scenes/Race/Zombi.tscn').instance()	
	town.name = nickname + "_" + str(Global_DataParser.rng.randi())
#	print(town.name)
	print("spawning Zombi in Grimeyard")
#	bag.name = nickname
	$'/root/Game/Empire'.add_child(town1)
	town1.position = $Sprite.global_position + Vector2(-128, -128) #$Sprite.global_position
	town1.set_target($Sprite.global_position)		
#	var bag_url_data = "res://Database//data_dark_fortress_" + town.name + ".json"
#	Global_DataParser.write_data(bag_url_data, data)
#	town.init(_inventory, town.name, data, die_position, bag_url_data)
#	town.reload_data_by_inventory(_inventory)
