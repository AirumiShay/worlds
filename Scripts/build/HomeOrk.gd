...
var spawn = false
var spawn_time = 300 #время рождения орков
var life_time = 180
var client = 0
var npc_god = false
...
func rpc_spawn_HomeOrk():
	if get_tree().is_network_server():	 
		var object_path = 'res://Scenes/Race/Ork_Warrior.tscn'
#		Global_DataParser.build_farm += 1		
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,$Sprite.global_position,"Ork_Warrior",Global_DataParser.join_race,Global_DataParser.location_number)
...
#	var	town = load('res://Scenes/Nature/Grass2.tscn').instance()	
	town.name = nickname + "_" + str(Global_DataParser.rng.randi())
#	print(town.name)

	$'/root/Game/Neutral'.add_child(town)
#	$'/root/Game/Nature'.add_child(town)
	town.position = self.position	
...
func _on_TimerLife_timeout():
	print("HomeOrk Life End") # Replace with function body.
	rpc_destroy_self()
...
func get_data():
	...
	_new_data["whoiam"] ="res://Scenes/Build/HomeOrk.tscn"	
	print(self.name)
#	print(self.name)	
	return _new_data
