...
var now_city = 0 # 0 - это хутор, 1 - это поселок  2 - это город 3 - это столица
func _ready():
	set_process(false)
...
#	$Owner.text = owner_name + ":" + name_farm
	rpc_spawn_mobs()
	update_hp()
	$New_Generation.value  = level_farm2 #  сколько простроено поселений жителями этого замка
	$Timer.start(spawn_time)
	Global_DataParser.build_fortress += 1	
#	Global_DataParser.build_castle_vampire += 1
	$TimerBegin.start(3)
	$SpriteBegin.visible = true
	print("new vampire fortress")
	print(_url_data)
...
func rpc_spawn_mobs(): #(nickname:String, data:Dictionary, die_position:Vector2, url_data:String="") -> void:
	if get_tree().is_network_server():
		var object_path ='res://Scenes/Race/Rogue.tscn'
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,$Sprite.global_position,"Peasant",Global_DataParser.join_race,location_number)
...
#	print("spawning stage 1")
	var town = load('res://Scenes/Race/Rogue.tscn').instance()
	town.name = nickname + "_" + str(Global_DataParser.rng.randi())
#	print(town.name)
	print("spawning from Vampire Castle")
#	bag.name = nickname
...
#		Global_DataParser.build_farm += 1		
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"FarmEmpire",Global_DataParser.join_race,location_number)
...
func rpc_spawn_capital(posXY): 
	if get_tree().is_network_server():	 
		...
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"CapitalEmpire",Global_DataParser.join_race,location_number)
	
func rpc_spawn_castle(posXY): 
	if get_tree().is_network_server():	 
		var object_path = 'res://Scenes/Build/VampireCastle.tscn'
		Global_DataParser.build_castle += 1			
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"CastleEmpire",Global_DataParser.join_race,location_number)
...
func get_data():
	...
	_new_data["whoiam"] ="res://Scenes/Build/VampireFortress.tscn"
	_new_data["nickname"] ="VampireFortress"
...
func _on_WhatTimer_timeout():
	if hp > 50:
		...
		if what_do == 1 and can_spawn == 0:				
			...
			$SpawnTime.text = str(int(can_spawn*spawn_time2))			
			can_spawn2 += 1
			if resource > 200 and (peasant_count > 30) and level_farm2 < 5:
				resource -= 200
				peasant_count -= 4
	#			if(already_build == false):			
	#				spawn = false
	#				already_build = true
				spawn_all_farm()						
	#	$WhatTimer.start(spawn_time)	
			elif what_do == 1 and resource > 500 and (peasant_count > 50) and level_farm2 == 3:
					resource -= 500
					peasant_count -= 25
					var x = int(position.x/256) - 1
					var y = int(position.y/256)	- 1
	...
	#				new_position += 1
			...
			elif what_do == 1 and resource >200 and (peasant_count > 30) and level_farm2 == 4:
					resource -= 200
					peasant_count -= 8
	#				var x = int(position.x/256) - 1
	#				var y = int(position.y/256)	- 1
...
#					$Owner.text = founder + ":" + name_farm #str(self.name)
					...
					$Sprite.texture = load("res://Assets/builds_new/vampire/unit_bld_priory.png")														
