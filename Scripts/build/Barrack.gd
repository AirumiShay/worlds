...
var spawn = false
var spawn_time = 300 #время рождения паладинов
var npc_god = false
...
#	print("after spawn")
	...
	spawn = true
	rpc_spawn_knight($Sprite.global_position)
	update_hp()		
...
#		print(kobold_count)
func rpc_spawn_knight(posXY): 
	if get_tree().is_network_server():	 
		var object_path = "res://Scenes/Race/Knight.tscn"
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,posXY,"Knight",Global_DataParser.join_race,location_number)
...
func spawn_mobs_local(nickname:String, die_position:Vector2): #, url_data:String="") -> void:
	if is_network_master():
		var town = load('res://Scenes/Race/Templar.tscn').instance()
		town.name = nickname + "_" + str(Global_DataParser.rng.randi())
#		print(town.position)
...
func get_data():
	...
	_new_data["nickname"] ="BarrackTemple"
	return _new_data
