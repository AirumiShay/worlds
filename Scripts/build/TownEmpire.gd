...
var location_number = Global_DataParser.location_number
...
func rpc_spawn_mobs(): #(nickname:String, data:Dictionary, die_position:Vector2, url_data:String="") -> void:
	if get_tree().is_network_server():
		var object_path ='res://Scenes/Race/Peasant.tscn'
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,self.position,"Peasant",Global_DataParser.join_race,location_number)
		return
...
#		print(kobold_count)
func rpc_spawn_grass():
	if get_tree().is_network_server():
		var object_path ='res://Scenes/Race/Grass2.tscn'
		Global_Network.spawn_object(get_tree().get_network_unique_id(),object_path,self.position,"Grass",Global_DataParser.join_race,location_number)
...
func rpc_spawn_fortress(posXY): 
	if get_tree().is_network_server():	 
		var object_path = "res://Scenes/Build/TowerGuard.tscn"
...
func rpc_spawn_magAcademy(posXY): 
	if get_tree().is_network_server():	 
		var object_path = "res://Scenes/Build/MagicAcademy.tscn"
...
func rpc_spawn_capital(posXY): 
	if get_tree().is_network_server():	 
		var object_path = 'res://Scenes/Build/Capital.tscn'
...
#						$Sprite.texture = load("res://Assets/builds_new/empire/unit_city.png")														
					...
					rpc_spawn_magAcademy(Vector2(a,b))
					level_farm2 += 1
				elif what_do == 4:
					rpc_spawn_magAcademy(Vector2(a,b))															
					level_farm2 += 1
...
func get_data():
	...
	_new_data["whoiam"] ="res://Scenes/Build/TownEmpire.tscn"
	_new_data["nickname"] ="TownEmpire"
