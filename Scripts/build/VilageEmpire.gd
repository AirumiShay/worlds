...
var spawn_time2 = 12 #время рождения животных и строительства поселков и замка
var npc_god = false
var new_position = 0
...
#		$SpawnTimer.start(spawn_time)
	...
	$Level.text = str(what_do) #wave_spawn)
	$Owner.text = owner_name
	rpc_spawn_mobs()
	update_hp()
	$New_Generation.value  = level_farm2 
	$Timer.start(spawn_time-1)
...
#	if(get_tree().is_network_server()): 
		...
		if new_position < 9:
			if new_position == 1:
				a = x  * 256
				b = (y - 1) * 256
				spawn_magtower("Village",Vector2(a,b))
			if new_position == 2:
				a = (x - 2)  * 256
				b = y  * 256
				spawn_storage("Village",Vector2(a,b))
			if new_position == 3:
				a = (x) * 256
				b = (y + 4)  * 256
...
func _on_WhatTimer_timeout():
	if what_do > 0:
		if resource > 100 and (peasant_count > 4):
			resource -= 100
			peasant_count -= 4
#			if(already_build == false):			
#				spawn = false
