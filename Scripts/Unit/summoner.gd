...
var max_build = 12 # максимум зданий для постройки
var build = 0 # сколько осталось построить зданий
var main_town # координаты, где появился этот юнит  summoner и куда возвращаться.
var builds_count = 0
...
#	$TimerNewBuild.start(30)
	main_town = position
#	print(main_town)
	main_town = $Sprite.global_position # get_parent().position
#	print (main_town)
func _process(delta):
...
#		var a = load("res://Scenes/DamageAreaLight.tscn").instance()
		var a = load("res://Scenes/Build/Avanpost.tscn").instance()
#		var dmg = 2
...
#		print(a.position)
		a.position = $Sprite.global_position #position #разброс попадания в цель
#		print(a.position)		
#		a.position = position + $DamagePos.position
#		print("Worker builded farm")	
#		print(build)
		builds_count += 1
		print("all builds from summoner = ")
		print(builds_count)
	else:
#		print("Not building")
...
#	build = max_build #снова можем строить
	attack_on()	
	$TimerNewBuild.start(60)	# Replace with function body.
