...
#заклинание , его создает Высший Вампир
var stands = true
...
#		if position.distance_to(pl.position) < 200:
		cancel_movement()
		speed = default_speed * 1 if speed == default_speed else speed
...
#		b.set_damage(dmg)
		...
		get_parent().add_child(a)
		b.position = $Sprite.global_position #$DamagePos.position #разброс попадания в цель
...
#		search_for_target()
		if target_count != null and target_count >  0:
			...
			var pl = pl1["position_target"]
			target_intercepted = $DamageNPC.get_pos_target() # pl
			if pl !=  Vector2():
				search_for_target()
