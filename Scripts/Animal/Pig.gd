...
var max_build = 3 # максимум зданий для постройки
var build = 3 # сколько осталось построить зданий
...
#	print ("ready for build")
	build = max_build
	attack_on()
	$TimerNewBuild.start(PiggySpawn)
...
#		var a = load("res://Scenes/DamageAreaLight.tscn").instance()
		var a = load("res://Scenes/Animal/Pig_small.tscn").instance()
#		var dmg = 2
...
#		print("Not building")
		print("Больше нельзя рожать поросят")
#		return
...
#	build = max_build #снова можем строить
	attack_on()	
	$TimerNewBuild.start(PiggySpawn)	# Replace with function body.
